...
static inline int memo_lookup(void *(*func)(void *), void *arg, void **result) {
  ...
  if (memo_table[h].valid && memo_table[h].func == func &&
...
// taille doit etre alignee sur 16 octets, meme chose que : sizeof(thread_s) %
// 16 == 0 ? sizeof(thread_s) : sizeof(thread_s) + 16 - sizeof(thread_s) % 16
...
#ifdef USE_CONTEXT
// pour que si un thread autre que main finit l'execution en dernier il peut
// free son stack son probleme
static char exitStack[8192];
// il faut lui donner ce contexte avec cette stack avant de free so stack
// "dynamique"
...
int thread_create(thread_t *createdThread, void *(*func)(void *), void *arg) {
  ...
  if (currentThread->func != NULL && currentThread->func == func) {
    ...
    if (currentThread->recursive_create_count >= 2 &&
        fibo_detected_func == NULL) {
  ...
  makecontext(&newThread->context, (void (*)(void))thread_wrapper, 1,
              newThread);
#else
#ifdef USE_PREEM
  unlock_preemption();
...
int thread_mutex_lock(thread_mutex_t *m)
{
    ...
    #endif
    //while (m->dummy) {
    if (__sync_lock_test_and_set(&m->dummy, 1)){ //0 si le mutex etait libre, 1 sinon
...
int thread_mutex_unlock(thread_mutex_t *m)
{
    ...
    #endif
    //m->dummy = 0;
    //s'il y a des threads en attente on les reveille sans liberer le mutex 
    if (!TAILQ_EMPTY((struct mutexQueue*) m->waitingQueue)) {
        ...
        __sync_lock_release(&m->dummy); //personne en attente, on libere le mutex
    }
    #ifdef USE_PREEM
        unlock_preemption();
    #endif
