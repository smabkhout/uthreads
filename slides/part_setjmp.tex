...
\begin{frame}{Performances : \texttt{ucontext} vs \texttt{setjmp/longjmp}}
  \vfill % Centrage vertical (pousse vers le bas)
  \begin{center}
      % On ajuste la largeur à 85% pour laisser respirer la slide
      \includegraphics[width=0.85\linewidth]{rapport-final/images/31-switch-many-context.png}
      
      \vspace{0.6cm} % Espace élégant entre l'image et la légende
      
      {\small\color{gray} $n \times 50$ changements de contexte}
  \end{center}
  \vfill % Centrage vertical (pousse vers le haut)
\end{frame}
...
\begin{tikzpicture}[
  ...
  \node[box, fill=teal!20]   (run) at (8.5, 1.4) {\texttt{run}};
  \node[box, fill=green!20]  (ex)  at (8.5, 0.2) {\texttt{exit}};
  \node[box, fill=orange!25] (fq)  at (5.5, 0.2) {\texttt{freeQueue}};

  \draw[arr] (cr)  -- (run);
  \draw[arr] (run) -- (ex);
  \draw[arr] (ex)  -- (fq)  node[midway, below, font=\tiny] {recycle};
...
\begin{frame}{Performances des optimisations}
  \vfill
  \begin{center}
      % Image centrée avec une largeur équilibrée
      \includegraphics[width=0.8\linewidth]{rapport-final/images/21-create-many-optim.png}
      
      \vspace{0.3cm}
      {\small Création de $n$ threads}
      
      \vspace{0.8cm} % Espace pour séparer l'analyse de l'image
      
      \begin{minipage}{0.9\linewidth}
          \centering\small
          \setbeamercolor{block title}{bg=white,fg=black}
          \textbf{\color{blue}recycle} = gain dominant (réutilisation des piles) \\
          \textbf{\color{gray}yields} = non affectés (pas d'allocation mémoire)
      \end{minipage}
  \end{center}
  \vfill
\end{frame}
