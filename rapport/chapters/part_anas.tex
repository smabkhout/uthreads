...
\end{itemize}
Nous avons retenu les \textit{TAILQ} de la bibliothèque \textit{OpenBSD} car
ils permettent un ordonnancement FIFO efficace et de supprimer n'importe quel
thread en $O(1)$, ce qui nous évitera des parcours linéaires qui vont affecter
les performances.
...
\begin{itemize}
    \item Structure : un entier (1 ou 0) + file d'attente \texttt{TAILQ} qui
    contient l'ensemble des threads qui attendent le déverrouillage de ce mutex.
...
\begin{itemize}
    ...
    \item \texttt{currentThread} : variable globale pointant vers le thread courant
\end{itemize}
