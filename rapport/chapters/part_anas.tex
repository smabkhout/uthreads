...
\subsubsection{Structures Fondamentales}
La structure \texttt{thread\_s} centralise toutes les informations :
\begin{itemize}
    \item Contexte d'exécution: (\texttt{ucontext\_t} + \texttt{jmp\_buf})
    \item Pile associée au thread (8 Ko)
    \item État (\texttt{READY/RUNNING/BLOCKED/TERMINATED})
    \item Valeur de retour et thread en attente (\texttt{joiningThread})
    \item Liaison TAILQ pour ordonnancement
\end{itemize}
...
\end{itemize}
La cible \texttt{make graphs} est en cours d'implémentation.

\subsection{Mutex}
Nous avons opté pour une implémentation simple mais efficace pour les mutex:
\begin{itemize}
    \item Structure : un entier (1 ou 0) + file d'attente TAILQ qui contient l'ensemble des threads qui attendent le déverouillage du mutex.
...
\begin{itemize}
    \item \texttt{readyQueue} (TAILQ) : threads prêts à s'exécuter (FIFO)
    \item \texttt{blockedQueue} (TAILQ) : threads en attente (\texttt{join})
    \item \texttt{currentThread} : varaible global pointant vers le thread courant
\end{itemize}

\subsection{Optimisations de Performance}
Pour améliorer davantage les performances, nous avons réussi à migrer vers \texttt{setjmp/longjmp}.