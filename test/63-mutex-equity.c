...
/* test pour verifier que prendre un mutex ne desactive pas l'equité envers les
 * autres threads. si ca deadlock, c'est qu'il y a un problème. la durée du test
 * n'est pas clairement utilisable pour juger de l'équité.
 *
...
static void *thfunc(void *dummy __attribute__((unused))) {
  ...
  while (fini != 0) {
    printf("  le fils yield avec le mutex en attendant que le compteur %u soit "
           "0\n",
           fini);
    thread_yield();
  }
