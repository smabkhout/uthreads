...
Bien que notre bibliothèque intègre déjà un ensemble solide de fonctionnalités avancées, 
telles que l'ordonnancement préemptif, la protection de la pile, 
la gestion des priorités et un mécanisme de signaux inter-threads, 
elle offre encore de nombreuses perspectives d'évolution.
...
confinant l'exécution à un unique cœur physique. 
Le support multi-cœurs permettrait d'exécuter nos threads en parallèle grâce à \texttt{pthreads} internes. 
Cette évolution nécessiterait de protéger nos structures (comme \texttt{readyQueue}) 
à l'aide de verrous(\textit{spinlocks}), ce qui peut toutefois limiter les gains de performance 
en cas de forte contention. Il faudrait également y intégrer des fonctions pour 
fixer un thread sur un cœur donné, évitant ainsi les migrations coûteuses et optimisant le tout.
...
équitable du temps processeur virtuel (\textit{vruntime}) entre tous les threads.
